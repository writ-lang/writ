...
(* [Cmd_show] — the [writ show] verb: print what a situation IS, addressed by the
   index the rest of the tool already uses.

   It exists because `writ derive` answers with state indices. A rules file that
   ...
   `writ query --at 17` for each — so the two instruments shared a numbering that
...
(* [--at N] repeated, or nothing at all — which shows the initial situation, the
   default [writ query] already uses for the same flag. *)
