...
(* The MCP server as a process: the only module in [mcp/] that touches a file
   ...
   The [resolve] callback is built HERE, over the same [Pol_loadpath] search
   order (design D3) the `pol` command line and the language server share: a
...
let resolve_for (base : string) : Pol_syntax.Loader.resolve =
 fun name ->
...
let respond line =
  ...
  | Ok msg ->
      Pol_mcp.Server.handle ~resolve:resolve_for ~version:Pol_mcp.Version.v msg
