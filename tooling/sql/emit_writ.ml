  ...
  if u.refs_total || u.refs_null || u.refs_mutable || u.refs_mutable_null then begin
    buf_add b
      "\n\
       ...
       ;; do; the export records the difference in a `-- writ:` pragma, because\n\
...
let emit_schema (b : Buffer.t) (name : string) (db : db) (u : usage) =
  ...
  buf_add b
    ...
    \  ;; nullable one costs a factor of two, which is the one distinction writ\n\
