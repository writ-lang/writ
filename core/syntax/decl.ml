...
let decode_arrow ~(dom : string) (d : Reader.t) :
  ...
  | Reader.List
      ...
      let scan c =
        match c with
        | Reader.List ([ Reader.Atom ("to", _); Reader.Atom (cd, _) ], _) ->
            cod := Some cd;
            Ok ()
        | Reader.List (Reader.Atom ("to", tp) :: _, _) ->
            Errors.err ~pos:tp
              "an arrow codomain must be a named type, not an inline set"
        | Reader.List ([ Reader.Atom ("of", _); Reader.Atom (dm, _) ], _) ->
            dom := dm;
            Ok ()
        | Reader.Atom ("fixed", _) ->
            fixed := true;
      ...
      match !cod with
      | None ->
          Errors.err ~pos:np ("arrow `" ^ name ^ "` has no (to …) codomain")
      | Some cod ->
          Ok
            {
              Schema.name;
...
let decode_type (d : Reader.t) :
    (Schema.ty * Schema.arrow list, Errors.t) result =
  ...
  | Reader.List (Reader.Atom ("type", _) :: Reader.Atom (name, _) :: clauses, _)
    -> (
      if List.exists is_arrow clauses then
        let* arrows = map_r (decode_arrow ~dom:name) clauses in
        Ok ({ Schema.name; flavor = Open; arrows }, arrows)
      else
        match clauses with
